...
public class AES {
    ...
    public static byte[] encrypt(byte[] data, byte[] key) {
        ...
        if (key.length != 32) {
            throw new RuntimeException("Invalid AES key length (must be 32 bytes)");
        }
        try {
            ...
            IvParameterSpec iv = new IvParameterSpec(ConfigureEncryptAndDecrypt.IVKey.getBytes());//使用CBC模式，需要一个向量iv，可增加加密算法的强度
    ...
    public static byte[] decrypt(byte[] data, byte[] key) {
        ...
        try {
            ...
            byte[] enCodeFormat = secretKey.getEncoded();
            SecretKeySpec seckey = new SecretKeySpec(enCodeFormat, "AES");
            Cipher cipher = Cipher.getInstance(ConfigureEncryptAndDecrypt.AES_ALGORITHM, "BC");// 创建密码器
            IvParameterSpec iv = new IvParameterSpec(ConfigureEncryptAndDecrypt.IVKey.getBytes());//使用CBC模式，需要一个向量iv，可增加加密算法的强度
